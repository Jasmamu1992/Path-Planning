            ...
            for (int i = 0; i < sensor_fusion.size(); i++){
                ...
                if((d < (2+4*lane+2)) && (d > (2+4*lane-2))){
                    ...
                    if ((check_car_s > car_s) && ((check_car_s-car_s)<25)){
                        too_close = true;
                        followspeed = check_speed;
                        planner.planned_state(predictions, lane, car_s);
                        cout << planner.state << endl;
                        planner.realize_state(lane);
                    }
                }
            }

            if (too_close){
                if (ref_vel > followspeed * 25 / 11){
                    cout << followspeed * 25 / 11 << endl;
                    ref_vel -= 0.224 * 3;
                }
            }
